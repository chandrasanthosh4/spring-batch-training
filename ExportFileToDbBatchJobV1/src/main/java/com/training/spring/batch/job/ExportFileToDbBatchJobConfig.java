...
public class ExportFileToDbBatchJobConfig {
	...
	public Job dbToFileJobConfiguration() throws Exception {
		return jobBuilderFactory.get("ExportFileToDbJob").validator(exportFileToDbJobParameterValidator)
				.start(fileToDbStepConfiguration()).build();
	}
	...
	public Step fileToDbStepConfiguration() throws Exception {
		return stepBuilderFactory.get("ExportFileToDbStep").<CustomerRecord, Customer>chunk(2)
				.reader(fileRecordReader(null)).processor(customerRecordProcessor).writer(dbRecordWriter())
				.faultTolerant().skipLimit(3).skip(ExportFileToDbJobBusinessException.class).allowStartIfComplete(false)
				.build();
	}
